...
import { deleteCoach, getCoachs } from "../../JS/Actions/coach";
...
const Coach = () => {
  ...
  const coachsList = useSelector((state) => state?.coachReducer?.coachsList);
  useEffect(() => {
    dispatch(getCoachs());
  }, [dispatch]);

  return (
    <div className="fullPagess">
      <NavBar />
          ...
          <Card className="card1" sx={{ maxWidth: 250 }}>
            ...
            <CardActions>
              <Button
                style={{ color: "red" }}
                size="small"
                onClick={() => dispatch(deleteCoach(coach._id))}
              >
                delete
              </Button>
              <EditCoach id={coach._id} />
            </CardActions>
          </Card>
        ))}
      </div>
