...
const pages = [<a href="/Member">Dashboard</a>];
...
const dispatch = useDispatch()
  return (
    <AppBar className="hey" position="static">
      <Container className="topBars" maxWidth="xl">
        ...
        <Tooltip title="Open settings">
          <IconButton onClick={handleOpenUserMenu} sx={{ p: 2 }}>
            <Avatar alt="Remy Sharp" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTsQ-YHX2i3RvTDDmpfnde4qyb2P8up7Wi3Ww&usqp=CAU" />
          </IconButton>
        </Tooltip>
        <Menu
          sx={{ mt: '45px' }}
        ...
        >
          {settings.map((setting) => (
            <MenuItem key={setting} onClick={handleCloseUserMenu}>
              <Typography  onClick={()=>dispatch(logout())} textAlign="center" color={ setting  === 'logout' ? 'error' : 'red'}>
                {setting}
              </Typography>
            </MenuItem>
          ))}
